...
export default async function ReceivablesPage({ searchParams }: { searchParams: Promise<{ mes?: string; personId?: string }> }) {
  ...
  const rows = visiblePeople.map(person => {
    ...
    const projectedItems = projectedInstallments.map(installment => ({ installment, amount: allocatedInstallmentAmount(installment, person.id) })).filter(item => item.amount !== 0);
    const projected = projectedItems.reduce((sum, item) => sum + item.amount, 0);
    const balance = charged - received;
    return { person, items, charged, received, balance, projectedItems, projected, totalPending: balance + projected };
  ...
  const totalPending = totalCharged - totalReceived + totalProjected;
  ...
  return <AppShell><section className="content"><header className="page-title"><div><p className="eyebrow">CONTAS A RECEBER</p><h1>Valores a receber</h1><p className="intro">Acompanhe despesas atribuídas, pagamentos e o saldo de cada pessoa.</p></div></header>
    ...
    <div className="summary-grid receivable-summary"><article className="summary-card"><p>Cobrado no mês</p><strong>{formatCents(totalCharged)}</strong><span>Despesas atribuídas</span></article><article className="summary-card"><p>Recebido no mês</p><strong>{formatCents(totalReceived)}</strong><span>Pagamentos registrados</span></article><article className="summary-card"><p>Saldo pendente</p><strong>{formatCents(totalPending)}</strong><span>Confirmado em aberto + previsto</span></article><article className="summary-card"><p>Previsto no mês</p><strong>{formatCents(totalProjected)}</strong><span>Parcelas futuras ainda não faturadas</span></article></div>
    <section className="panel receivable-main"><div className="panel-heading"><div><p className="eyebrow">POR PESSOA</p><h2>Fechamento mensal</h2></div></div>{rows.length ? <div className="receivable-people">{rows.map(row => <details key={row.person.id} open={Boolean(filters.personId)}><summary><span className="avatar">{row.person.name[0]}</span><div><strong>{row.person.nickname || row.person.name}</strong><small>{row.items.length} lançamento(s) · recebido {formatCents(row.received)}{row.projected > 0 ? ` · previsto ${formatCents(row.projected)} · total ${formatCents(row.totalPending)}` : ""}</small></div><b className={row.totalPending < 0 ? "refund-value" : ""}>{row.totalPending > 0 ? `${formatCents(row.totalPending)} em aberto` : row.charged || row.received ? "Quitado" : "Sem fatura ainda"}</b></summary><div className="receivable-settle"><span>{row.balance > 0 ? `Saldo pendente de ${formatCents(row.balance)} confirmado${row.projected > 0 ? ` (+ ${formatCents(row.projected)} previsto, total ${formatCents(row.totalPending)})` : ""}` : row.charged || row.received ? `Pagamento de ${formatCents(row.received)} registrado nesta competência${row.projected > 0 ? ` · previsto ${formatCents(row.projected)}` : ""}` : row.projected > 0 ? `Nenhuma despesa faturada ainda, mas há ${formatCents(row.projected)} previsto para esta competência` : "Nenhuma despesa faturada ainda nesta competência"}</span>{row.balance > 0 && <form action={settleMonthlyBalance}><input type="hidden" name="personId" value={row.person.id} /><input type="hidden" name="mes" value={period.selected} /><button className="primary" type="submit">Quitar saldo do mês</button></form>}</div><div className="receivable-items">{row.items.map(({ transaction, amount }) => <article key={transaction.id}><div><strong>{transaction.description}</strong><small>{transaction.occurredAt.toLocaleDateString("pt-BR")} · {transaction.category?.name || "Sem categoria"} · {transaction.card?.name || "Sem cartão"}</small></div><b>{formatCents(amount)}</b></article>)}</div>{row.projectedItems.length > 0 && <div className="receivable-forecast"><p className="eyebrow">PREVISTO PARA {monthLabel.toUpperCase()} · AINDA NÃO FATURADO</p><div className="receivable-items">{row.projectedItems.map(({ installment, amount }) => <article key={installment.id}><div><strong>{installment.plan.description}</strong><small>Parcela {installment.sequence}/{installment.plan.totalInstallments} · {installment.plan.category?.name || "Sem categoria"} · vence {installment.estimatedDueDate?.toLocaleDateString("pt-BR") || "a definir"}</small></div><b>{formatCents(amount)}</b></article>)}</div></div>}</details>)}</div> : <div className="empty-state compact"><strong>Nenhum valor encontrado</strong><span>Não há despesas, pagamentos ou parcelas previstas para este filtro no mês escolhido.</span></div>}</section>
    </section></AppShell>;
}
