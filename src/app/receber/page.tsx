...
export default async function ReceivablesPage({ searchParams }: { searchParams: Promise<{ mes?: string; personId?: string }> }) {
  ...
  const rows = visiblePeople.map(person => {
    ...
    return { person, items, charged, received, balance: charged - received };
  }).filter(row => row.charged !== 0 || row.received !== 0);
  ...
  return <AppShell><section className="content"><header className="page-title"><div><p className="eyebrow">CONTAS A RECEBER</p><h1>Valores a receber</h1><p className="intro">Acompanhe despesas atribuídas, pagamentos e o saldo de cada pessoa.</p></div></header>
    ...
    <div className="summary-grid receivable-summary"><article className="summary-card"><p>Cobrado no mês</p><strong>{formatCents(totalCharged)}</strong><span>Despesas atribuídas</span></article><article className="summary-card"><p>Recebido no mês</p><strong>{formatCents(totalReceived)}</strong><span>Pagamentos registrados</span></article><article className="summary-card"><p>Saldo pendente</p><strong>{formatCents(totalCharged - totalReceived)}</strong><span>A receber nesta competência</span></article></div>
    <section className="panel receivable-main"><div className="panel-heading"><div><p className="eyebrow">POR PESSOA</p><h2>Fechamento mensal</h2></div></div>{rows.length ? <div className="receivable-people">{rows.map(row => <details key={row.person.id} open={Boolean(filters.personId)}><summary><span className="avatar">{row.person.name[0]}</span><div><strong>{row.person.nickname || row.person.name}</strong><small>{row.items.length} lançamento(s) · recebido {formatCents(row.received)}</small></div><b className={row.balance < 0 ? "refund-value" : ""}>{row.balance > 0 ? `${formatCents(row.balance)} em aberto` : "Quitado"}</b></summary><div className="receivable-settle"><span>{row.balance > 0 ? `Saldo pendente de ${formatCents(row.balance)}` : `Pagamento de ${formatCents(row.received)} registrado nesta competência`}</span>{row.balance > 0 && <form action={settleMonthlyBalance}><input type="hidden" name="personId" value={row.person.id} /><input type="hidden" name="mes" value={period.selected} /><button className="primary" type="submit">Quitar saldo do mês</button></form>}</div><div className="receivable-items">{row.items.map(({ transaction, amount }) => <article key={transaction.id}><div><strong>{transaction.description}</strong><small>{transaction.occurredAt.toLocaleDateString("pt-BR")} · {transaction.category?.name || "Sem categoria"} · {transaction.card?.name || "Sem cartão"}</small></div><b>{formatCents(amount)}</b></article>)}</div></details>)}</div> : <div className="empty-state compact"><strong>Nenhum valor encontrado</strong><span>Não há despesas ou pagamentos para este filtro no mês escolhido.</span></div>}</section>
    </section></AppShell>;
}
