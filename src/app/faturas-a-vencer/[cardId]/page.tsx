...
import { bulkClassifyDueItems, bulkDeleteDueItems, createDueCategory, deleteDueInstallment, deleteDueTransaction, updateDueItemField, updateDueItemShares, updateProjectedDueDate } from "./actions";
...
export default async function DueInvoiceDetailPage({ params, searchParams }: { params: Promise<{ cardId: string }>; searchParams: Promise<{ mes?: string }> }) {
  ...
  const [card, transactions, installments, people, categories, merchantRules] = await Promise.all([
    ...
    prisma.merchantRule.findMany({ where: { active: true }, select: { pattern: true, categoryId: true, personId: true } }),
  ]);
  ...
  const deleteItem = deleteDueTransaction.bind(null, cardId);
  const deleteProjected = deleteDueInstallment.bind(null, cardId);
  const bulkDelete = bulkDeleteDueItems.bind(null, cardId);
