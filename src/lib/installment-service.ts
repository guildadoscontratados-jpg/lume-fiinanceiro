...
export async function createOrConfirmImportedTransaction(db: Db, input: { cardId: string; invoiceId: string | null; description: string; occurredAt: Date; amountCents: number; installmentNo: number | null; installmentTotal: number | null; billingPeriod?: BillingPeriod | null; billingPeriods?: Array<BillingPeriod & { sequence: number }> | null; invoiceMonth?: Date | null; personId: string | null; categoryId: string | null; shares: Share[]; notes: string }) {
  ...
  let plan = await db.installmentPlan.findFirst({ where: { cardId: input.cardId, merchantNormalized, totalInstallments: input.installmentTotal!, installmentCents: input.amountCents, startDate }, include: { installments: true } });
  if (!plan) {
    const endDate = monthStart(new Date(Date.UTC(lastPeriod.billingYear, lastPeriod.billingMonth - 1, 1, 12)));
    plan = await db.installmentPlan.create({ data: { description: input.description, merchantNormalized, totalCents: input.amountCents * input.installmentTotal!, installmentCents: input.amountCents, totalInstallments: input.installmentTotal!, startDate, endDate, cardId: input.cardId, personId: input.personId, categoryId: input.categoryId, installments: { create: periods.map(period => ({ sequence: period.sequence, dueMonth: monthStart(new Date(Date.UTC(period.billingYear, period.billingMonth - 1, 1, 12))), amountCents: input.amountCents, status: period.sequence === input.installmentNo ? "CONFIRMED" : period.sequence > input.installmentNo! ? "PROJECTED" : "CANCELLED", purchaseDate: input.occurredAt, billingMonth: period.billingMonth, billingYear: period.billingYear, billingReference: period.billingReference, estimatedDueDate: period.estimatedDueDate, expectedAmountCents: input.amountCents, source: "IMPORT" })) } }, include: { installments: true } });
  }
