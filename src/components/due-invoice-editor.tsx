...
export function DueInvoiceEditor({ title, rows, projected, people, categories, merchantRules, bulkAction: serverBulkAction, shareAction: serverShareAction, itemAction: serverItemAction, projectedAction: serverProjectedAction, automationAction }: { title: string; rows: Row[]; projected: Projected[]; people: Person[]; categories: Category[]; merchantRules: MerchantRule[]; bulkAction: ServerAction; shareAction: ServerAction; itemAction: ServerAction; projectedAction: ServerAction; automationAction: (pattern: string, categoryId: string) => Promise<void> }) {
