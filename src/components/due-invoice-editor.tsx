...
export function DueInvoiceEditor({ title, rows, projected, people, categories, merchantRules, bulkAction: serverBulkAction, shareAction: serverShareAction, itemAction: serverItemAction, projectedAction: serverProjectedAction, deleteAction: serverDeleteAction, deleteProjectedAction: serverDeleteProjectedAction, bulkDeleteAction: serverBulkDeleteAction, automationAction }: { title: string; rows: Row[]; projected: Projected[]; people: Person[]; categories: Category[]; merchantRules: MerchantRule[]; bulkAction: ServerAction; shareAction: ServerAction; itemAction: ServerAction; projectedAction: ServerAction; deleteAction: ServerAction; deleteProjectedAction: ServerAction; bulkDeleteAction: ServerAction; automationAction: (pattern: string, categoryId: string) => Promise<void> }) {
  ...
  const [bulkShareCount, setBulkShareCount] = useState(1);
  ...
  const deleteAction = async (formData: FormData) => runRowAction(serverDeleteAction, formData);
  const deleteProjectedAction = async (formData: FormData) => runRowAction(serverDeleteProjectedAction, formData);
  const bulkDeleteAction = async (formData: FormData) => runRowAction(serverBulkDeleteAction, formData);
  const confirmDelete = (event: MouseEvent<HTMLButtonElement>, label: string) => { if (!window.confirm(`Excluir "${label}"? Essa ação não pode ser desfeita.`)) event.preventDefault(); };
  const confirmBulkDelete = (event: MouseEvent<HTMLButtonElement>) => { if (!window.confirm("Excluir os lançamentos selecionados? Essa ação não pode ser desfeita.")) event.preventDefault(); };
  ...
  const submitShareScope = (event: MouseEvent<HTMLButtonElement>, installment: boolean) => {
    ...
    scope.value = installment && window.confirm("Deseja aplicar este rateio a todas as parcelas deste parcelamento?\n\nOK: todas as parcelas.\nCancelar: somente esta parcela.") ? "ALL" : "ONLY_THIS";
  };
  const submitInlineForm = (form: HTMLFormElement, installment: boolean) => {
    const scope = form.elements.namedItem("scope") as HTMLInputElement;
  ...
  const submitInline = (event: ChangeEvent<HTMLSelectElement | HTMLInputElement>, installment: boolean) => {
    const form = event.currentTarget.form; if (!form) return;
    submitInlineForm(form, installment);
  };
  ...
  const maybeRegisterAutomation = async (description: string, categoryId: string) => {
  ...
  return <section className="panel detail-panel">
    ...
    {rows.length || projected.length ? <form id="due-bulk-form" action={bulkAction} className="due-bulk-toolbar"><select name="bulkAction" required value={bulkOperation} onChange={event => { setBulkOperation(event.currentTarget.value); setBulkShareCount(1); }}><option value="" disabled>Alteração em massa...</option><option value="CATEGORY">Definir categoria</option><option value="PERSON">Definir devedor</option><option value="SHARE">Definir rateio</option><option value="DUE_MONTH">Definir vencimento</option></select>{bulkOperation === "DUE_MONTH" ? <label className="due-month-field"><span>Novo vencimento</span><input name="dueMonth" type="month" required /></label> : bulkOperation === "SHARE" ? <div className="due-share-fields"><input type="hidden" name="shareCount" value={bulkShareCount} />{Array.from({ length: bulkShareCount }, (_, index) => <label key={index}><select name={`sharePerson-${index}`} required defaultValue=""><option value="">Selecione</option>{people.map(person => <option key={person.id} value={person.id}>{person.nickname || person.name}</option>)}</select>{bulkShareCount > 1 && <><input name={`sharePercent-${index}`} inputMode="decimal" placeholder="%" required /><span>%</span></>}{index === 0 ? <button type="button" className="assignment-plus" disabled={bulkShareCount >= people.length} onClick={() => setBulkShareCount(count => Math.min(people.length, count + 1))} aria-label="Adicionar pessoa">+</button> : <button type="button" className="assignment-minus" onClick={() => setBulkShareCount(count => Math.max(1, count - 1))} aria-label="Remover pessoa">−</button>}</label>)}</div> : <><select name="categoryId"><option value="">Categoria...</option><CategoryOptions categories={categories} /></select><select name="personId"><option value="">Devedor...</option>{people.map(person => <option key={person.id} value={person.id}>{person.nickname || person.name}</option>)}</select></>}<button className="primary" type="submit">Aplicar aos selecionados</button><button className="danger-button" type="submit" formAction={bulkDeleteAction} formNoValidate onClick={confirmBulkDelete}>Excluir selecionados</button></form> : null}
    {rows.length || projected.length ? <div className="detail-table-wrap"><table className="detail-table due-editor-table"><thead><tr><th>Sel.</th><th>Data</th><th>Estabelecimento</th><th>Devedor / rateio</th><th>Categoria</th><th>Parcela</th><th>Valor</th><th>Vencimento</th><th>Status</th><th>Ações</th></tr></thead><tbody>
