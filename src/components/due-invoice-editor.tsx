...
export function DueInvoiceEditor({ title, rows, projected, people, categories, bulkAction: serverBulkAction, shareAction: serverShareAction, itemAction: serverItemAction, projectedAction: serverProjectedAction }: { title: string; rows: Row[]; projected: Projected[]; people: Person[]; categories: Category[]; bulkAction: ServerAction; shareAction: ServerAction; itemAction: ServerAction; projectedAction: ServerAction }) {
  ...
  const bulkAction = async (formData: FormData) => {
    ...
    if (operation === "DUE_MONTH") {
      ...
      const hasInstallment = rows.some(row => selectedIds.has(row.id) && row.hasInstallment);
      if (hasInstallment) {
  ...
  return <section className="panel detail-panel">
    ...
    {rows.length ? <form id="due-bulk-form" action={bulkAction} className="due-bulk-toolbar"><select name="bulkAction" required value={bulkOperation} onChange={event => setBulkOperation(event.currentTarget.value)}><option value="" disabled>Alteração em massa...</option><option value="CATEGORY">Definir categoria</option><option value="PERSON">Definir devedor</option><option value="DUE_MONTH">Definir vencimento</option></select>{bulkOperation === "DUE_MONTH" ? <label className="due-month-field"><span>Novo vencimento</span><input name="dueMonth" type="month" required /></label> : <><select name="categoryId"><option value="">Categoria...</option>{categories.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select><select name="personId"><option value="">Devedor...</option>{people.map(person => <option key={person.id} value={person.id}>{person.nickname || person.name}</option>)}</select></>}<button className="primary" type="submit">Aplicar aos selecionados</button></form> : null}
